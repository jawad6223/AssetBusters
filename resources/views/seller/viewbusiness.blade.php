    ...
    <div class="row">       
      ...
      <div class="col-md-9">
        <table class="manage-table responsive-table">
         ...
         <tr>
           <td class="utf-title-container"><img src="{{asset('storage/app/'. $business->primaryimage)}}" style="height:100px" alt="">
             <div class="title">
               <h4><a href="#">{{$business->title}}</a></h4>
               <span>{{$business->street}},{{$business->city}},{{$business->state}},{{$business->zip}},{{$business->country}}

               </span> <span class="table-property-price">$ {{$business->total_price}}</span> 
       </div>
     </td>
           <td class="utf-expire-date">{{ date('d-m-Y', strtotime($business->created_at));}}</td>
           <td class="action">
       <a href="{{url('seller/readbusiness/'.$business->id)}}" class="view tooltip left" title="View"><i class="icon-feather-eye"></i></a>
       <a href="{{url('seller/editbusiness/' .$business->id)}}" class="edit tooltip left" title="Edit"><i class="icon-feather-edit"></i></a> 
       <a href="{{url('seller/deletebusiness/' .$business->id)}}" class="delete tooltip left" title="Delete"><i class="icon-feather-trash-2"></i></a>
     </td>
         </tr>

         @endforeach
        </table>
        <a href="{{url('seller/addbusiness')}}" class="utf-centered-button margin-top-30 button">Add New Business</a> 
	  </div>
    </div>
  </div>
