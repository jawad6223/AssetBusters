    ...
    <div class="row">       
      ...
      <div class="col-md-9">
        <table class="manage-table responsive-table">
          ...
          <tr>
            <td class="utf-title-container"><img src="{{asset('storage/app/'. $property->favourite_property->primaryimage)}}" style="height:100px" alt="">
              <div class="title">
                <h4><a href="#">{{$property->favourite_property->title}}</a></h4>
                <span>{{$property->favourite_property->street}},{{$property->favourite_property->city}},{{$property->favourite_property->state}},{{$property->favourite_property->zip}},{{$property->favourite_property->country}}

                </span> <span class="table-property-price">$ {{$property->favourite_property->price}}</span> 
			  </div>
			</td>
            <td class="utf-expire-date">{{ date('d-m-Y', strtotime($property->favourite_property->created_at));}}</td>
            <td class="action">
				<a href="{{url('propertydetail/'.$property->favourite_property->id)}}" class="view tooltip left" title="View"><i class="icon-feather-eye"></i></a>
				<a href="{{url('propertyfavourite/' . $property->favourite_property->id)}}" class="delete tooltip left" title="Delete"><i class="icon-feather-trash-2"></i></a>
